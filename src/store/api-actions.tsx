import {ApiRoute, SHOW_PAGE, SortParamApi} from '../constants/constants';
...
export const fetchGuitars = (page: number, sort?: string, order?: string): any => (dispatch: AppDispatch, _getState: State, api: AxiosInstance) => {
  let url = `${ApiRoute.Guitars}?_start=${SHOW_PAGE * page - SHOW_PAGE}&_limit=${SHOW_PAGE}&_embed=comments`;

  if (sort) {
    url += `&${SortParamApi.Sort}=${sort}`;
  }

  if (order) {
    url += `&${SortParamApi.Order}=${order}`;
  }
